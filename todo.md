...
# ARCHITECTURE: See docs/TENANCY_DESIGN.md (rev 5 — fourth security review incorporated)
...
# 43. Outbox deduplication_key NOT NULL with dedup_company_guard generated column

# 44. Company-wide visibility = regardless of farm assignment, NOT regardless of RBAC

# 45. primary_email_identity_id FK: UNIQUE(user_id, id) on user_identities (target-side)

# 46. authentication_tokens composite FK (user_id, user_identity_id) → user_identities

# 47. audit_log.company_id nullable permanently (auth/platform events before company selection)

# 48. company_invitations: owner excluded from role enum, pending_invitation_guard for dedup

# 49. company_invitations.accepted_by_user_id with FK to users (was ambiguous accepted_by)

# 50. platform_admins.role has NO default — explicit role required for grant

# 51. FK deletion matrix: RESTRICT for business, SET NULL for actors, CASCADE for sessions

# 52. Company deletion purge order (dependency-ordered, audit records preserved)

# 53. File upload: link table created in same tx as pending attachment (no auth gap)

# 54. MFA freshness: privileged_session_max_age checked per-company on sensitive actions

# 55. expenses.scope_type + farm_id consistency enforced by DB CHECK constraint

# 56. role_permissions.page renamed to resource BEFORE unique constraint creation

# 57. Migration: global uniques dropped only in Phase 7 (after tenant-aware proven)

# 58. Migration: users.role mapped via explicit reviewed mapping table (not direct copy)

# 59. users.email_normalized: backfill, resolve duplicates, then add UNIQUE

# 60. No JWT references in design (opaque sessions only)
...
- [ ] Add `email_normalized`, `primary_email_identity_id` (composite FK to user_identities(user_id, id) via UNIQUE(user_id, id) on target), `auth_version`, `failed_login_attempts`, `locked_until`, `last_password_change` to `users` (NO email_verified — derived from primary identity)
- [ ] Backfill `email_normalized`, resolve duplicates, then add `UNIQUE(email_normalized)`
- [ ] Add `action_category` (nullable initially), `membership_id`, `session_id`, `request_id`, `outcome` to `audit_log` (company_id nullable permanently — auth/platform events occur before company selection; two logical stores: tenant_audit_log with company_id NOT NULL, platform_security_log with company_id NULL)
- [ ] Add `effect ENUM('allow','deny')` to `role_permissions` (rename `page` to `resource` BEFORE creating unique constraint); UNIQUE without `effect` column
- [ ] Add `user_identity_id` to `authentication_tokens` with composite FK `(user_id, user_identity_id) → user_identities(user_id, id)` (binds token to specific identity for verify_email/identity_link)
- [ ] Add all FKs to new tables with explicit ON DELETE behavior per FK deletion matrix (RESTRICT for business, SET NULL for actors, CASCADE for sessions/credentials)
- [ ] Actor columns reference memberships via composite FKs (created_by_membership_id, etc.) not global users
- [ ] `company_invitations`: role enum excludes `owner`, `accepted_by_user_id` with FK to users, `pending_invitation_guard` generated column for duplicate prevention, `email_normalized` column
- [ ] `expenses.scope_type` + `farm_id` consistency enforced by DB CHECK constraint
- [ ] `user_identities` has `UNIQUE(user_id, id)` as composite FK target for `users.primary_email_identity_id`
- [ ] Create `platform_admins` table (replaces deprecated `users.role` for platform authorization; NO DEFAULT on role — explicit role required, granted_by NOT NULL)
...
- [ ] Rename `role_permissions.page` to `resource` BEFORE creating unique constraint: `ALTER TABLE role_permissions CHANGE COLUMN page resource VARCHAR(100) NOT NULL;`
...
- [ ] `users.role` deprecated — replaced by `platform_admins` table (no DEFAULT on role); legacy role removed from all authorization paths
...
- [ ] MFA freshness: check `mfa_verified_at` against each company policy's `privileged_session_max_age` on every sensitive action (billing, export, ownership transfer, API-key, MFA changes) — per-company, not global
...
- [ ] Upload pipeline: create pending `file_attachments` AND pending link table entry in SAME transaction (immutable upload-intent; no auth gap during quarantine)
- [ ] Do not allow attachment target entity to change after issuing upload URL
...
- [ ] Outbox: encrypted_payload for tokens (envelope encryption, erased after sending), worker leasing (locked_by/locked_until), `deduplication_key` NOT NULL DEFAULT '' with `dedup_company_guard` generated column for per-company uniqueness (avoids MySQL NULL-in-UNIQUE semantics)
...
- [ ] Phase 1: Create new tables, add nullable companyId + farmId + public_id, add composite unique keys alongside existing (do NOT drop old constraints yet); audit_log.company_id nullable permanently AND action_category nullable; role_permissions.company_id nullable; user_identities with UNIQUE(user_id, id)
...
- [ ] Phase 3: Backfill historical rows in bounded batches (1000 rows), generate ULIDs, backfill email_normalized + resolve duplicates, run catch-up backfill for rows inserted during backfill, verify zero null rows + zero orphans
- [ ] Phase 4: Add composite FKs (including farm_id FKs on historical tables), make companyId NOT NULL, create and validate tenant-aware unique constraints, add generated-column uniques, add owner_company_guard, add current_company_guard, backfill action_category then make NOT NULL, rename role_permissions.page→resource, add expenses CHECK constraint, add all FKs with explicit ON DELETE per deletion matrix
...
- [ ] Phase 6: Add email/password auth, create user_identities for OAuth users, create company_users for existing users using explicit reviewed mapping table (NOT direct users.role copy — manually inspect privileged users), migrate to opaque sessions, create platform_admins table, deprecate users.role, add UNIQUE(email_normalized)
- [ ] Phase 7: Verify counts, enable new-tenant registration, remove legacy code, drop old global unique indexes (ONLY now — after tenant-aware constraints proven in production), rehearse rollback
...
- [ ] All FKs have explicit ON DELETE behavior per FK deletion matrix (RESTRICT/SET NULL/CASCADE)
...
- [ ] Auth tokens bound to `user_identity_id` via composite FK for verify_email/identity_link
...
- [ ] Outbox `deduplication_key` NOT NULL with `dedup_company_guard` generated column
...
- [ ] `audit_log.company_id` nullable permanently (auth/platform events before company selection)
- [ ] `company_invitations.role` excludes `owner` (DB-enforced); `pending_invitation_guard` for dedup
- [ ] `company_invitations.accepted_by_user_id` has FK to users
- [ ] `expenses.scope_type` + `farm_id` consistency enforced by DB CHECK
- [ ] File upload: link table created in same tx as pending attachment (no auth gap)
...
- [ ] `user_identities` has `UNIQUE(user_id, id)` as composite FK target
- [ ] `authentication_tokens` composite FK `(user_id, user_identity_id) → user_identities`
- [ ] `platform_admins.role` has NO default (explicit role required)
- [ ] FK deletion matrix defined (RESTRICT/SET NULL/CASCADE per category)
- [ ] Company deletion purge order defined (dependency-ordered, audit preserved)
- [ ] `role_permissions.page` renamed to `resource` before unique constraint
- [ ] Migration: global uniques dropped only in Phase 7 (after tenant-aware proven)
- [ ] Migration: `users.role` mapped via explicit reviewed mapping table
- [ ] Migration: `role_permissions.company_id` and `audit_log.action_category` added nullable first
- [ ] No JWT references in design (opaque sessions only)
...
## Phase 56: Five-Workspace UX + Canonical Data Redesign

### 56.1 Structural UX Foundation

- [x] Add one typed route manifest shared by routing, permissions, labels, icons, and navigation
- [x] Add temporary `current | simple` design-version provider persisted as `lfms-design-version`
- [x] Add user-menu design switch without coupling it to light/dark theme
- [x] Replace imperative sidebar navigation buttons with real links
- [x] Remove nested `DashboardLayout` from Recycle Bin
- [x] Ensure exactly one application `<main>` landmark and add a skip-to-content target
- [x] Make desktop/mobile navigation content independently scrollable without footer overlap
- [x] Remove viewport zoom restriction and restore browser zoom accessibility
- [x] Add visible permission-loading state instead of a blank page

### 56.2 Shared Simple-Design Components

- [x] Create `AppShell`, `PageShell`, `PageHeader`, `PageToolbar`, and `FilterBar`
- [x] Create reusable `StatCard`, paginated `DataTable`, and `MobileRecordCard`
- [x] Create accessible `FormDialog`, `EmptyState`, and permission-aware `PageActions`
- [x] Consolidate theme ownership so application UI and toast notifications use one source
- [x] Install `gsap` + `@gsap/react`; remove unused Framer Motion after usage audit
- [x] Add reduced-motion-aware 150–250ms transitions for shell, dialogs, drawers, and record updates

### 56.3 Five Workspaces

- [x] Home: attention queue, four compact KPIs, and daily quick actions
- [ ] Animals: paginated registry, saved views, optional columns, mobile cards, and unified animal detail
  - [x] Add simple registry with 25-row pagination and essential default columns
  - [x] Add responsive mobile record cards and accessible sorting
  - [x] Add persisted optional columns without duplicating canonical data
  - [ ] Add named saved views and URL-backed filter state
  - [ ] Replace profile tabs with one unified animal timeline/detail view
- [ ] Operations: Feed, Health, Breeding, Growth, Expenses, and Sales internal views
- [ ] Finance: Overview, Animal Profitability, Income Statement, Receivables, and Reports
- [ ] Administration: Farm Setup, Users, Data, Audit, Recycle Bin, and Settings
- [ ] Redirect legacy URLs to the correct workspace/view while preserving deep links
- [ ] Store workspace view, filter, sort, and pagination state in URL parameters

### 56.4 Canonical Data and API Cleanup

- [ ] Document canonical source versus derived values for every user-visible field
- [ ] Derive age, days on farm, current weight, target progress, ADG, stock, days remaining, shrinkage, outstanding balance, price/kg, and P&L
- [ ] Add `sale_payments` as canonical payment history and migrate `sales.amountPaid`
- [ ] Make sale creation drive sold status/exit date without duplicate user input
- [ ] Keep lambing history immutable and link promoted animals instead of duplicating editable birth facts
- [ ] Add unified animal-events endpoint for weight, health, breeding, expense, status, and sale history
- [ ] Add server pagination/filtering to Animals, P&L, Audit, Notifications, Sales, Feed, and Recycle Bin
- [ ] Centralize Dashboard, Profitability, and Income Statement calculations in one finance service
- [ ] Update Excel/PDF/import/export contracts with compatibility reads for deprecated fields

### 56.5 Page Migration Order

- [x] Dashboard/Home
- [ ] Animals registry and unified animal timeline
- [ ] Growth/Fattening saved view
- [ ] Health/Vaccinations
- [ ] Breeding/Lambing
- [ ] Feed/Inventory
- [ ] Expenses and Sales/Payments
- [ ] Finance workspace
- [ ] Administration/Farm Setup
- [ ] Recycle Bin and Audit

### 56.6 Accessibility, Localization, and Performance

- [ ] Associate every form label with its field and provide dialog descriptions
- [ ] Make rows, sortable headers, actions, and navigation keyboard-operable
- [ ] Complete Arabic translation for values, statuses, charts, units, finance terms, and empty states
- [ ] Verify RTL layouts without negative/off-screen tables
- [ ] Use mobile cards instead of desktop-width tables
- [ ] Default large datasets to 25 server-paginated records
- [ ] Avoid animating tables or large record collections
- [ ] Test reduced motion and low-end mobile performance

### 56.7 Rollout and Acceptance

- [ ] Verify Current and Simple designs use identical permissions, APIs, records, and mutations
- [ ] Verify canonical-data migration preserves existing financial and operational totals
- [ ] Test English/Arabic, RTL/LTR, light/dark, desktop/tablet/mobile
- [ ] Run unit, regression, accessibility, performance, code, and security reviews per migration slice
- [ ] Make Simple design default after acceptance
- [ ] Remove Current design and temporary switch after the compatibility period
