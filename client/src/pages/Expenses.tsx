...
import { DollarSign, Plus, Trash2, AlertTriangle, Pencil } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function EditExpenseDialog({ expense, onSuccess }: { expense: any; onSuccess: () => void }) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    expenseDate: new Date(expense.expense.expenseDate).toISOString().split("T")[0],
    categoryId: String(expense.expense.categoryId),
    subCategoryId: expense.expense.subCategoryId ? String(expense.expense.subCategoryId) : "",
    amount: String(expense.expense.amount),
    targetType: expense.expense.targetType,
    headId: expense.expense.headId ? String(expense.expense.headId) : "",
    categoryTarget: expense.expense.categoryTarget ? String(expense.expense.categoryTarget) : "",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md w-[95vw] sm:w-auto max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)}>{t("common.cancel")}</Button>
          <Button onClick={handleSubmit} disabled={updateExpense.isPending}>
            {updateExpense.isPending ? "Saving..." : t("common.save")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
